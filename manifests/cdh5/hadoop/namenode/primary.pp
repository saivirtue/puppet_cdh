# == Class puppet_cdh::cdh5::hadoop::namenode::primary
# Hadoop Primary NameNode.
#
# This class is applied by puppet_cdh::cdh5::hadoop::master even
# if we aren't using HA Standby Namenodes.  The primary NameNode will be inferred
# as the first entry $puppet_cdh::cdh5::hadoop::namenode_hosts variable.  If we are using
...
#
class puppet_cdh::cdh5::hadoop::namenode::primary inherits puppet_cdh::cdh5::hadoop::namenode {
    # Go ahead and transision this primary namenode to active if we are using HA.
    if ($::puppet_cdh::cdh5::hadoop::ha_enabled) {
        $primary_namenode_id = $::puppet_cdh::cdh5::hadoop::primary_namenode_id

        exec { 'haaadmin-transitionToActive':
            # $namenode_id is set in parent puppet_cdh::cdh5::hadoop::namenode class.
        ...
        Puppet_cdh::Cdh5::Hadoop::Directory {
    ...
    puppet_cdh::cdh5::hadoop::directory { '/tmp':
        owner => 'hdfs',
        group => 'hdfs',
        mode  => '1777',
    }
    ...
    puppet_cdh::cdh5::hadoop::directory { '/user':
        owner => 'hdfs',
        group => 'hadoop',
        mode  => '0775',
    }
    ...
    puppet_cdh::cdh5::hadoop::directory { '/user/hdfs':
        owner   => 'hdfs',
        group   => 'hdfs',
        mode    => '0755',
        require => Puppet_cdh::Cdh5::Hadoop::Directory['/user'],
    }

    # sudo -u hdfs hdfs dfs -mkdir /var
    puppet_cdh::cdh5::hadoop::directory { '/var':
        owner => 'hdfs',
        group => 'hdfs',
        mode  => '0755',
    }
    ...
    puppet_cdh::cdh5::hadoop::directory { '/var/lib':
        owner   => 'hdfs',
        group   => 'hdfs',
        mode    => '0755',
        require => Puppet_cdh::Cdh5::Hadoop::Directory['/var'],
    }

    # sudo -u hdfs hdfs dfs -mkdir /var/log
    puppet_cdh::cdh5::hadoop::directory { '/var/log':
        owner   => 'hdfs',
        group   => 'hdfs',
        mode    => '0755',
        require => Puppet_cdh::Cdh5::Hadoop::Directory['/var'],
