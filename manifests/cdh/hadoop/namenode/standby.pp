# == Class puppet_cdh::cdh::hadoop::namenode::standby
...
#
class puppet_cdh::cdh::hadoop::namenode::standby inherits puppet_cdh::cdh::hadoop::namenode {
    # Fail if nameservice_id isn't set.
    if (!$ha_enabled) {
        fail('Cannot use Standby NameNode in a non HA setup.  Specify journalnodes in the $journalnode_hosts parameter on the puppet_cdh::cdh::hadoop::params class to enable HA.')
    }
